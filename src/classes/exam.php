...
class Exam{
 public function getAddQuestion($data){
   $quesNo = mysqli_real_escape_string($this->db->connection, $data['quesNo']);
   $ques   = mysqli_real_escape_string($this->db->connection, $data['ques']);
   $ans    = array();
   $ans[1] = $data['ans1'];
   $ans[2] = $data['ans2'];
   $ans[3] = $data['ans3'];
   $ans[4] = $data['ans4'];
   $rightAns = mysqli_real_escape_string($this->db->connection, $data['rightAns']);
